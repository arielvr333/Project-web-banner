...
io.on('connection', function (socket) {
    ...
    //var idAddress = socket.handshake.address;
    ...
    socket.on('disconnect', function () {
        MongoClient.connect(url, function (err, db) {
            var dbo = db.db("MongoDataBase");
            var obj = { user: idAddress, action: "disconnected", time: datetime };
...
app.get('/screen-1', function (request, response) {
    connectToSocket(response, 'screen-1');
});

app.get('/screen-2', function (request, response) {
    connectToSocket(response, 'screen-2');
});

app.get('/screen-3', function (request, response) {
    connectToSocket(response, 'screen-3');
});
 ...
 function connectToSocket(response, screenName) {
     io.sockets.on('connection', function (socket) {
        ...
        MongoClient.connect(url, function (err, db) {
            var dbo = db.db("MongoDataBase");
            var datetime = new Date().toString().slice(0, 24);
            var obj = { user: ip, action: screenName, time: datetime };
            dbo.collection("usersData").insertOne(obj, function (err, res) { if (err) throw err; });
            var query = { _id: screenName };
            dbo.collection("advertisements").find(query).toArray(function (err, result) {
                if (err) throw err;
        ...
        socket.on('disconnect', function () {
            MongoClient.connect(url, function (err, db) {
                var dbo = db.db("MongoDataBase");
                var datetime = new Date().toString().slice(0, 24);
                var obj = { user: ip, action: "disconnected", time: datetime };
                dbo.collection("usersData").insertOne(obj, function (err, res) { if (err) throw err; });
            });
        });
    });
...
app.get('/get-data', checkAuthenticated, function (req, res, next) {
    var resultArray = [];
